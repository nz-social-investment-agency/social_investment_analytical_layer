...
OUTPUT: [&schema.].SIAL_MOH_pfhd_events
...
execute(
create view [&schema.].SIAL_MOH_pfhd_events as (
	select 
		[snz_uid], 
		department,
		datamart,
	...
	from (
			...
			from &idi_refresh..[moh_clean].[pub_fund_hosp_discharges_event] pfhd
			left join IDI_Sandpit.[&schema.].[moh_pu_pricing] pu on 
	...
	group by [snz_uid], department,
		...
		execute(select top 10 * from [&schema.].SIAL_MOH_pfhd_events) by odbc;
