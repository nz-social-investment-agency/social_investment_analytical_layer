...
OUTPUT: [&schema.].[SIAL_moj_courtcase_events]
...
August 2019 - PNH Added a select statement to log if user does not have access to underlying tables
*********************************************************************************************************/
...
create view [&schema.].[SIAL_MOJ_courtcase_events] as
select 
...
from (
	...
	from (
		...
		from (
			...
			from &idi_refresh..[moj_clean].[charges] c
			inner join [IDI_Metadata].[clean_read_CLASSIFICATIONS].[moj_court_id] court1 on (c.[moj_chg_last_court_id_code] = court1.court_id)
			left join IDI_Sandpit.[&schema.].[moj_offence_to_category_map] offcatmap on (c.moj_chg_offence_code = offcatmap.offence_code)
		) unordered_charges
	)ordered_charges 
...
left join IDI_Sandpit.[&schema.].[moj_offense_cat_pricing] pricing on (cases.offence_category = pricing.offence_category and cases.court_type = pricing.court_type and 
													cases.end_date between pricing.start_date and pricing.end_date);	) by odbc;

	execute(select top 10 * from [&schema.].[SIAL_MOJ_courtcase_events]) by odbc;
